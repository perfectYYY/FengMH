...
- `0x16 WHEEL_TEST` 提供固定腿轮驱、零电流自由推动、独立参数原地 TROT 轮驱三种测试模式；`0x18` 调步态参数，模式 3 下 `0x8A` 以 25 Hz 输出步态诊断；500 ms 未刷新会自动退出并在当前位置锁轮。
- 完整上位机版本发送 `0x80` 整机状态、`0x81` 电机轮询、`0x86/0x87` 机械臂反馈，并保留 `0x88/0x89` 底盘诊断；所有上行均通过非阻塞 USB 队列发送。
