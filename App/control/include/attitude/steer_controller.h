 ...
 *
 * 模式:
 *   STEER_MODE_OFF       — wz 直通, 不干预步态
 *   STEER_MODE_ABSOLUTE  — 跟踪上位机给出的绝对偏航角
 *   STEER_MODE_AUTO_HOLD — 锁存直行开始时的偏航角
 *
 * PID 控制律:
 *   yaw_err = wrap_pi(target - current)
 *   wz_out  = kp * yaw_err + ki * ∫err - kd * yaw_rate  (clamped)
 */
...
typedef enum {
    ...
    STEER_MODE_ABSOLUTE = 1,  /* 上位机绝对目标偏航角 */
    STEER_MODE_AUTO_HOLD = 2, /* 锁存当前偏航角 */
} steer_mode_t;
 ...
 *   current_yaw_rate: 当前偏航角速度 (rad/s)
 *   dt_s:        控制周期 (秒)
